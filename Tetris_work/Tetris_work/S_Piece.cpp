...
void SPiece::spawnPiece(GameBoard &gameArray)
{
...
void SPiece::resetPiece(char color)
{
	*this = SPiece(color);
}
...
void SPiece::move(GameBoard &gameArray, int direction, bool &placed, int stage)
{
	if (stage == 1 || stage == 3)
	{
		switch (direction) {
...
void SPiece::rotate(GameBoard &gameArray, int stage, bool &placed)
{
	if (stage == 1 || stage == 3)
	{
		if (!isCollision(gameArray, DOWN, placed))
